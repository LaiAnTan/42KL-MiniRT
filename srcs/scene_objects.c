...
t_camera	*scene_new_camera(int cam_fov, double cam_coords[3], double cam_vec_orient[3])
{
	...
	new_camera->cam_coords = init_vector_from_array(cam_coords);
	new_camera->cam_vec_orient = init_vector_from_array(cam_vec_orient);
...
t_light		*scene_new_light(int l_rgb[3], double l_coords[3], double l_brightness)
{
	...
	new_light->l_coords = init_vector_from_array(l_coords);
...
t_sphere	*scene_new_sphere(int sp_rgb[3], double sp_diameter, double sp_coords[3])
{
	...
	new_sphere->sp_coords = init_vector_from_array(sp_coords);
...
t_plane		*scene_new_plane(int pl_rgb[3], double pl_coords[3], double pl_vec_normal[3])
{
	...
	new_plane->pl_coords = init_vector_from_array(pl_coords);
	new_plane->pl_vec_normal = init_vector_from_array(pl_vec_normal);
	new_plane->next = NULL;
	return (new_plane);
}
...
t_cylinder	*scene_new_cylinder(int cy_rgb[3], double cy_height, double cy_diameter, double cy_coords[3], double cy_vec_axis[3])
{
	...
	new_cylinder->cy_coords = init_vector_from_array(cy_coords);
	new_cylinder->cy_vec_axis = init_vector_from_array(cy_vec_axis);
...
void	scene_free_camera_list(t_camera *camera_list_head)
{
	...
	{
		temp = curr->next;
		free_vector(&curr->cam_coords);
		free_vector(&curr->cam_vec_orient);
		free(curr);
		curr = temp;
	}
}
...
void	scene_free_plane_list(t_plane *plane_list_head)
{
	...
	{
		temp = curr->next;
		free_vector(&curr->pl_coords);
		free_vector(&curr->pl_vec_normal);
		free(curr->pl_rgb);
		free(curr);
		curr = temp;
	}
...
void	scene_free_cylinder_list(t_cylinder *cylinder_list_head)
{
	...
	{
		temp = curr->next;
		free_vector(&curr->cy_coords);
		free_vector(&curr->cy_vec_axis);
		free(curr->cy_rgb);
		free(curr);
		curr = temp;
	}
...
{
	printf("cam_coords = ");
	print_vector(camera->cam_coords);
	printf("cam_vec_orient = ");
	print_vector(camera->cam_vec_orient);
	printf("cam_fov = %d\n", camera->cam_fov);
}
...
{
	printf("pl_coords = ");
	print_vector(plane->pl_coords);
	printf("pl_vec_normal = ");
	print_vector(plane->pl_vec_normal);
...
{
	printf("cy_coords = ");
	print_vector(cylinder->cy_coords);
	printf("cy_vec_axis = ");
	print_vector(cylinder->cy_vec_axis);
